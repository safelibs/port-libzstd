...
`safe/build.rs` wires Cargo features into cfgs and build metadata. It enables `libzstd_threading` by default unless the static/no-MT variant features request otherwise (`safe/build.rs:36-65`), sets `LIBZSTD_THREADING`, `LIBZSTD_VARIANT_SUFFIX`, and `LIBZSTD_DEFAULT_ARTIFACT` environment values (`safe/build.rs:83-88`), and gives the `cdylib` an ELF SONAME of `libzstd.so.1` (`safe/build.rs:89`). Its only C compilation jobs are bounded ABI glue: it resolves upstream legacy sources from an optional in-tree source override or from `../original/libzstd-1.5.5+dfsg2/lib` (`safe/build.rs:18-24`, `safe/build.rs:91-102`) and compiles `error_private.c`, `xxhash.c`, `zstd_v05.c`, `zstd_v06.c`, `zstd_v07.c`, and `safe/src/ffi/legacy_shim.c` with hidden symbol visibility and `ZSTD_LEGACY_SUPPORT=5` (`safe/build.rs:96-125`).

Cargo features are defined in `safe/Cargo.toml:23-30`: the default feature set is empty, `legacy` is a named but empty feature, and `threading`, `build-shared-default`, `build-static-default`, `variant-mt`, and `variant-nomt` drive build metadata and cfg selection. `safe/scripts/build-artifacts.sh` builds separate shared and static artifacts with Cargo, installs `libzstd.so.1.5.5`, `libzstd.so.1`, `libzstd.so`, the static `libzstd.a`, and the checked-in headers, and generates pkg-config and CMake install files from `safe/pkgconfig/` and `safe/cmake/`. Static artifacts are built through a cached local copy of the active Rust sysroot where the GNU thread minimum-stack helper returns `PTHREAD_STACK_MIN`; this removes the optional weak loader lookup code at source compile time instead of editing the produced archive.
...
| Sites | Justification |
| --- | --- |
| `safe/src/ffi/types.rs:45,47` | Defines upstream-compatible custom allocator callback types as `unsafe extern "C" fn` because Rust cannot verify arbitrary C callback contracts. |
| `safe/src/ffi/types.rs:488` | Defines the upstream sequence-producer callback type as `unsafe extern "C" fn` to match the published advanced compression ABI. |
| `safe/src/ffi/compress.rs:3957` | Calls the registered sequence-producer callback after building the C argument list from the current compression context. |
...
The port's intended public FFI boundary is the original libzstd C ABI/API exposed through the 185 `ZSTD_*` and `ZDICT_*` exports and the checked-in headers under `safe/include/`. Source-level evidence from `rg -n 'extern "C"' safe/src safe/include safe/tests safe/build.rs --glob '!target/**' --glob '!out/**'` shows that the first-party Rust sources contain public libzstd exports, the original ABI callback typedefs in `safe/src/ffi/types.rs:45,47,488`, and one non-public `unsafe extern "C"` block in `safe/src/decompress/legacy.rs:12`. A maintained-source scan found no direct Rust source calls to libc, OS syscalls, pthreads, mmap, malloc, or dynamic loading APIs.
...
- `safe/scripts/build-artifacts.sh:262-305` builds shared and static artifacts with `cargo rustc`, installs `libzstd.so.1.5.5`, `libzstd.so.1`, `libzstd.so`, and the normalized static `libzstd.a`, and `safe/scripts/build-artifacts.sh:307-387` installs the checked-in headers plus generated pkg-config and CMake metadata from `safe/pkgconfig/` and `safe/cmake/`.
- `safe/scripts/build-original-cli-against-safe.sh:324-356` installs safe headers/libraries into the helper tree, writes a helper-only static-name redirect to the safe shared object, builds upstream `programs/` and `contrib/pzstd/`, and verifies that `zstd` and `pzstd` resolve libzstd from the helper root.
...
grep -RIn '<runtime-import-token-pattern>' safe
rg -n '<runtime-import-token-pattern>' safe/src safe/build.rs safe/Cargo.toml --glob '!target/**' --glob '!out/**'
...
rg -n '<deleted-upstream-fallback-token-pattern>' safe --glob '!target/**' --glob '!out/**' --glob '!PORT.md'
