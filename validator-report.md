...
Post-correction validator rerun at `safe/out/validator/artifacts/port-04-test/results/libzstd/summary.json`: 85 cases, 5 source cases, 80 usage cases, 82 passed, 3 failed, 85 casts, validator runner status 1. The current failed testcases are `usage-libarchive-tools-zstd-space-file-extract`, `usage-libarchive-tools-zstd-two-space-files-list`, and `usage-libarchive-tools-zstd-two-topdirs-list`; `check-validator-phase-results.py` passed after these usage rows were recorded and reported them as allowed remaining failed testcases for `impl_validator_libarchive_usage_regressions`.
